...
const updateProduct = async () => {
  ...
  const productData: Omit<Product, 'id'> & { id?: string } = {
    ...
    contificoId: formData.value.contificoId,
  ...
  <v-dialog v-model="isOpen" max-width="600" persistent>
    <v-card>
      ...
      <v-card-actions class="pa-6 pt-0">
        <v-spacer />
        <v-btn variant="outlined" color="grey-darken-1" @click="closeDialog" :disabled="isLoading"> Cancelar </v-btn>
        <v-btn color="primary" @click="updateProduct" :loading="isLoading"> Actualizar Producto </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
