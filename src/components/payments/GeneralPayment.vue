...
const emit = defineEmits(['update:modelValue', 'update:selectedParticipant', 'update:selectedProduct', 'update:notes', 'update:selectedDiscount', 'update:selectedCampus']);
...
<template>
    <div class="general-payment-container">
        <div class="tw:grid tw:grid-cols-2 tw:gap-6">
            <div>
                <h3 class="tw:text-lg tw:font-semibold mb-4">Facturado a</h3>
                <VCombobox @update:search="searchClient" v-model="selectedParticipant" :items="participantsList"
                    item-title="name" item-value="id" variant="outlined" density="compact"
                    placeholder="Seleccionar cliente" return-object @update:model-value="handleParticipantChange">
        ...
        <div class="tw:grid tw:grid-cols-2 tw:gap-6">
            <div>
                ...
                <VCombobox @update:search="searchProduct" v-model="selectedProduct" :items="productsList"
                    item-title="name" item-value="id" variant="outlined" density="compact" placeholder="Buscar producto"
                    return-object @update:model-value="handleProductChange">
                    <template v-slot:item="{ props, item }">
                        <v-list-item v-bind="props">
                            <v-list-item-subtitle>
                                {{ item.raw?.basePrice ? `$${item.raw.basePrice} ${item.raw.currency || 'USD'}` :
                                    'Sinprecio' }} -
            ...
            <div>
                ...
                <v-select v-model="selectedDiscount" :items="discountsList" item-title="name" item-value="id"
                    variant="outlined" density="compact" placeholder="Seleccionar descuento" return-object clearable
                    @update:model-value="handleDiscountChange">
                    <template v-slot:item="{ props, item }">
                        <v-list-item v-bind="props">
                            <v-list-item-title class="tw:flex tw:items-center tw:gap-2">
                                <span>{{ item.raw?.discountValue }}
                                    {{ item.raw?.discountType === 'P' ? '%' : (item.raw?.discountType === 'E' ? '$'
                                        : '') }}</span>
                            </v-list-item-title>
                        </v-list-item>
                    </template>
            ...
            <div class="tw:flex tw:flex-col tw:gap-4">
                <div class="tw:flex tw:justify-between tw:items-start">
                    <div>
                        ...
                        <p v-if="selectedProductDetails.description" class="tw:text-sm tw:text-gray-600 tw:mt-1">
                            Descripción: {{
                                selectedProductDetails.description }}</p>
                    </div>
                    <div class="tw:text-right">
                        <div class="tw:text-lg tw:font-bold">${{ selectedProductDetails.price }} {{
                            selectedProductDetails.currency }}</div>
                ...
                <div v-if="selectedProductDetails.programs && selectedProductDetails.programs.length > 0" class="mt-2">
                    ...
                    <div class="tw:flex tw:flex-wrap tw:gap-2">
                        <v-chip v-for="prog in selectedProductDetails.programs" :key="prog.id" size="small"
        ...
        <div v-else
            class="mt-4 tw:border tw:border-gray-200 tw:rounded-md p-4 tw:text-center tw:text-gray-500 py-8 mb-3">
        ...
        <div>
            ...
            <v-textarea v-model="notes" variant="outlined" placeholder="Añadir notas o términos de pago (opcional)"
                rows="4" hint="Incluya instrucciones de pago o información adicional para el cliente"
                @update:model-value="emit('update:notes', notes)"></v-textarea>
        </div>
    </div>
</template>
