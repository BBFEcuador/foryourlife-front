...
const onRemoveParticipant = async (id: string) => {
    ...
    if (result.isConfirmed) {
        removeParticipantsMutations.mutate({ url: "removeVisionaries", teamId: props.team.id, users: [{ id } as Participant] }, {
            onError(e) {
                const error = e as AxiosError<ErrorApiResponse>
                showErrorToast(error)
            },
            onSuccess(_,v,__) {
                props.team.visionaries = props.team.visionaries.filter(x => x.id != v.users[0].id) 
    ...
    <v-data-table :headers="headers" :show-select="isForEdit" :search="search" :items="team.visionaries" :loading="isTeamLoading">
        <template #top>
            <v-toolbar class="px-6 tw:bg-gradient-to-r tw:from-white tw:to-gray-50/50" flat v-motion
                ...
                <div class="tw:flex-1 tw:max-w-md tw:relative">
                    <VTextField v-model="search" placeholder="Buscar participantes..." variant="outlined"
        ...
        <template #loading>
            ...
            <div class="tw:flex tw:justify-center tw:py-12 tw:bg-gray-50/30">
                <div
                    class="tw:flex tw:items-center tw:gap-4 tw:bg-white tw:px-6 tw:py-4 tw:rounded-2xl tw:shadow-lg">
                ...
                <div class="tw:relative tw:z-10">
                    ...
                    <div class="tw:absolute tw:-top-1 tw:-right-1">
                        <div class="tw:relative">
                            ...
                            <div
                                class="tw:absolute tw:inset-0 tw:bg-red-500 tw:opacity-25 tw:blur-sm tw:rounded-full">
                            </div>
                        </div>
                    </div>
                </div>
