...
<template>
  <v-card variant="flat">
    <v-progress-circular v-if="availableStaffMutations.isPending.value" indeterminate color="primary"></v-progress-circular>
    <v-alert v-else-if="availableStaffMutations.isError.value" type="error" class="mb-4"> Error al cargar los participantes </v-alert>
    <div v-else>
