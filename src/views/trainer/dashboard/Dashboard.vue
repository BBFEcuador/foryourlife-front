...
const getLocalCardImage = (idx: number): string => {
  const imgs = shuffledBlogImages.value.length
    ? shuffledBlogImages.value
  ...
  <div>
    <UiParentCard title="Mis Entrenamientos">
      <v-row>
              ...
              <v-col cols="6" sm="6">
                <v-card-item class="pb-3">
                  ...
                  <div class="text-caption textSecondary">
                    Inicio: {{ new Date(team.training.startDate).toLocaleDateString() }}
                  </div>
                  <div class="text-caption textSecondary">
                    Fin: {{ new Date(team.training.endDate).toLocaleDateString() }}
                  </div>
                  <div class="text-caption textSecondary">
                    Sede: {{ team.training?.campus?.city ?? 'N/A' }}
                  </div>
                  <div class="text-caption textSecondary">
                    Participantes: {{ team.users?.length ?? 0 }}
                  </div>
                  <div class="d-flex align-center justify-center mt-8 ">
                      <v-chip class="text-success">
                        ACTIVO
                      </v-chip>
                    <v-btn size="23" icon variant="flat" class="grey100 ml-4" @click="router.push({ name: 'trainer-team', params: { id: team.id } })">
        ...
        <v-col v-if="!store.trainer?.teams || store.trainer.teams.length === 0" cols="12" class="tw-text-center tw-py-10">
