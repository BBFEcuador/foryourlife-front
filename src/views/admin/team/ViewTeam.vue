...
const handleGenerateMasiveList = async () => {
  const teamId = props.team.id;
  if (!teamId) return;
  // loadingContractId.value = teamId;
  try {
    const data = await generateMasiveListMutation.mutateAsync(teamId);
    const blob = new Blob([new Uint8Array(data)], {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `Listado_${props.team.trainingData?.name}_${new Date().toLocaleDateString('es-EC').replace(/\//g, '-')}.xlsx`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    toast.success('Listado generado exitosamente');
  } catch (error) {
    const err = error as AxiosError<{ message: string }>;
    toast.error(err.response?.data?.message || 'Error al generar listado');
  } finally {
    // loadingContractId.value = null;
  }
};
...
<template>
      ...
      <VCol cols="12" md="9" sm="12" class="tw:gap-4">
        <div class="tw:text-end mb-4 mt-4">
          ...
          <VBtn
            class="mb-1 ml-1"
            color="info"
            @click="handleGenerateMasiveList()"
            :loading="generateMasiveListMutation.isPending.value"
            :disabled="generateMasiveListMutation.isPending.value"
          >
        ...
        <v-card variant="outlined" elevation="0" class="bg-surface" rounded="lg">
          <v-tabs v-model="tab">
            ...
            <v-tab
              value="4"
              v-if="
                team.trainingData?.curseLevel == 'LIFE' ||
                team.trainingData?.curseLevel == 'LIFE_2' ||
                team.trainingData?.curseLevel == 'LIFE_3'
              "
              >Master life</v-tab
            >
          </v-tabs>
  ...
  <AddMembersTeam
    :model-value="showAddMembersDialog"
    @cancel="showAddMembersDialog = false"
