  ...
  if (data.page) {
    if (data.itemsPerPage != perPage.value) {
      if (data.itemsPerPage == -1) {
        perPage.value = teamsData.value.totalElements;
      } else {
...
<template>
  ...
  <div v-if="checkPermission(PermissionEnum.SEE_TEAMS)">
    <VRow v-auto-animate>
      <VCol cols="12">
              ...
              >
                <template #top>
                  ...
                  >
                    ...
                    <VSpacer />
                    <VBtn
                      v-if="checkPermission(PermissionEnum.CREATE_TEAMS)"
                      variant="elevated"
                      color="primary"
                    ...
                    >
                      <div class="tw:relative">
                        ...
                        <div class="tw:absolute tw:inset-0 tw:bg-white tw:opacity-25 tw:blur-sm tw:rounded-full tw:animate-pulse"></div>
                      </div>
                      Nuevo Equipo
                    </VBtn>
                  </v-toolbar>
                ...
                <template #item.name="{ item }">
                  <div class="tw:flex tw:items-center tw:gap-3 tw:text-nowrap group">
                    <Icon icon="mdi:account-group" class="tw:text-primary tw:transition-transform group-hover:tw:scale-110" />
                    <div
                ...
                <template #item.trainer.name="{ item }">
                  <div class="tw:flex tw:items-center tw:gap-2 tw:text-nowrap group">
                    <div class="tw:bg-amber-50 tw:p-2 tw:rounded-lg tw:transition-all group-hover:tw:bg-amber-100">
                      <Icon icon="mdi:account-tie" class="tw:text-amber-600 tw:transition-transform group-hover:tw:scale-110" />
                    </div>
                    <div>
                      <span class="tw:font-medium tw:text-gray-800 group-hover:tw:text-amber-600 tw:transition-colors">{{
                        item.trainer.name
                      }}</span>
                      <div class="tw:text-xs tw:text-gray-500 group-hover:tw:opacity-100 tw:transition-opacity">
                        {{ item.trainer.email }}
                      </div>
                    </div>
                  </div>
                ...
                <template #item.training.courseLevel="{ item }">
                  <div class="tw:flex tw:items-center tw:gap-2 group">
                    <VChip
                      :color="getLevelColor(item.training.courseLevel)"
                      variant="flat"
                    ...
                    >
                      <div class="tw:relative">
                        <Icon
                          :icon="getLevelIcon(item.training.courseLevel)"
                          height="20"
                ...
                <template #item.training.campus.city="{ item }">
                  <div class="tw:flex tw:items-center tw:gap-2 tw:text-nowrap group">
                    <div class="tw:bg-blue-50 tw:p-2 tw:rounded-lg tw:transition-all group-hover:tw:bg-blue-100">
                      <Icon icon="mdi:map-marker" class="tw:text-blue-600 tw:transition-transform group-hover:tw:scale-110" />
                    </div>
                    <span class="tw:font-medium tw:text-gray-700 group-hover:tw:text-blue-600 tw:transition-colors">{{
                      item.training.campus.city
                    }}</span>
                  </div>
                </template>
                ...
                <template #item.trainingData.startDate="{ item }">
                  <div v-if="item.trainingData" class="tw:flex tw:items-center tw:gap-2 tw:text-nowrap group">
                    <div class="tw:bg-green-50 tw:p-2 tw:rounded-lg tw:transition-all group-hover:tw:bg-green-100">
                      <Icon icon="mdi:calendar-clock" class="tw:text-green-600 tw:transition-transform group-hover:tw:scale-110" />
                    </div>
                    <div>
                      <span class="tw:font-medium tw:text-gray-800 group-hover:tw:text-green-600 tw:transition-colors">{{
                        formatDate(item.trainingData.startDate)
                      }}</span>
                    </div>
                  </div>
                </template>
