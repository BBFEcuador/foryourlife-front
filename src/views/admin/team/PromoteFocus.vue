...
import type { TeamYourPromotionRequest } from '@/models/Team';
...
const { isTeamError, isTeamLoading, team, refetchTeam, promotionYourRequest } = useAdminTeam(route.params.id.toString());
...
const handleTrainerSelected = (trainer: Trainers[]) => {
  ...
  promotionYourRequest.value.trainer = trainer[0].id;
...
const onPromoteTeam = () => {
  promoteToYourMutation.mutate(promotionYourRequest.value);
};
        ...
        <v-card elevation="2" class="tw:rounded-xl tw:overflow-hidden tw:border tw:border-gray-200">
          <v-card-item>
            <v-card-title class="d-flex align-center tw:gap-4">
              ...
              <v-btn color="primary" variant="flat" class="tw:rounded-lg" @click="showConfirmDialog = true">
  ...
  <v-dialog v-model="showConfirmDialog" width="400">
    <v-card class="tw:rounded-lg">
      ...
      <v-card-actions class="tw:p-4 tw:bg-gray-50">
        <v-spacer></v-spacer>
        <v-btn color="grey" variant="text" @click="showConfirmDialog = false" class="tw:mr-2"> Cancelar </v-btn>
        <v-btn color="primary" variant="flat" :loading="promoteToYourMutation.isPending.value" @click="onPromoteTeam" > Confirmar promoción </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
