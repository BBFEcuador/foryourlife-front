...
watch([promoteToLifeMutation.isError, promoteToLife2Mutation.isError, promoteToLife3Mutation.isError, promoteToLifeGraduateMutation.isError], () => {
    const mutation = getPromotionMutation();
...
watch([promoteToLifeMutation.isSuccess, promoteToLife2Mutation.isSuccess, promoteToLife3Mutation.isSuccess, promoteToLifeGraduateMutation.isSuccess], () => {
    const mutation = getPromotionMutation();
                ...
                <v-card elevation="2" class="tw:rounded-xl tw:overflow-hidden tw:border tw:border-gray-200">
                    <v-card-item>
                        <v-card-title class="d-flex align-center tw:gap-4">
                            ...
                            <v-btn v-if="team.training.courseLevel === 'LIFE_3'" color="primary" variant="flat"
                                class="tw:rounded-lg" @click="showConfirmDialog = true">
                                GRADUAR EL EQUIPO 🚀
                            </v-btn>
    ...
    <v-dialog v-model="showConfirmDialog" width="500">
        <v-card class="rounded-lg">
            ...
            <v-card-actions class="p-4">
                <v-spacer></v-spacer>
                <v-btn color="error" variant="tonal" @click="showConfirmDialog = false" class="mr-2">
                    Cancelar
                </v-btn>
