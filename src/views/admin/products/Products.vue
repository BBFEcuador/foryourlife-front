...
<template>
  ...
  <div v-if="checkPermission(PermissionEnum.SEE_PRODUCTS)">
    <v-row>
      <v-col cols="12">
          ...
          <VDataTableServer :items="productsData.content" :headers="headers" :search="productSearch"
            ...
            <template #item.contificoId="{ item }">
              <div class="d-flex tw:justify-center">
                ...
                <Icon :icon="item.contificoId ? 'mdi:check-circle' : 'mdi:close-circle'"
                  :class="item.contificoId ? 'tw:text-green-800' : 'text-error'" height="25" />
              </div>
            </template>
    ...
    <CreateProduct v-if="checkPermission(PermissionEnum.CREATE_PRODUCTS)" :model-value="showCreateDialog"
      :is-loading="saveProductMutations.isPending.value" @cancel="showCreateDialog = false" @save="saveProduct" />
