...
<template>
  ...
  <v-row>
    <v-col cols="12">
      <UiParentCard title="Lista de usuarios">
        <v-data-table :headers="headers" :search="search" :items="campus" :loading="isFetching">
          <template v-slot:top>
            <v-toolbar
              ...
              :duration="250"
            >
              <VTextField hide-details placeholder="Buscar Sede" prepend-inner-icon="mdi-magnify" v-model="search" class="custom-card" />
              <v-spacer></v-spacer>
            </v-toolbar>
          </template>
          <template v-slot:item.actions="{ item }">
            <div class="d-flex ga-2">
              <v-btn icon color="primary" @click="editCampus()" size="small">
                <Icon icon="mdi-edit" height="18" />
              </v-btn>
              <v-btn small color="error" @click="deleteCampus()" icon size="small">
