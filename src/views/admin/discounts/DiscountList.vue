    ...
    Swal.fire({
        ...
        if (params.isConfirmed) {
            try {
                ...
                Swal.fire(
                    '¡Éxito!',
                    `El descuento ha sido ${!isCurrentlyActive ? 'activado' : 'desactivado'} correctamente.`,
                    'success'
                );
            } catch (error) {
...
<template>
    ...
    <v-row>
        <v-col cols="12">
            <UiParentCard title="Lista de Descuentos">
                <v-data-table-server :headers="headers" :search="search" :items="discountsData.content"
                    :loading="isDiscountsLoading" :items-length="discountsData.totalElements" :items-per-page="10"
                    ...
                    <template v-slot:top>
                        <v-toolbar class="px-6 tw:bg-gradient-to-r tw:from-white tw:to-gray-50/50" flat v-motion
                            ...
                            <v-spacer></v-spacer>
                            <VBtn variant="elevated" color="primary" @click="onCreateDiscount">
                    ...
                    <template #item.discountType="{ item }">
                        <VChip :color="item.discountType === 'P' ? 'primary' : 'info'" size="small" variant="flat"
                            class="!tw:font-medium tw:text-xs !tw:min-w-[100px] tw:justify-center" :class="{
                                'tw:bg-blue-50 !tw:text-blue-700': item.discountType === 'P',
                                'tw:bg-cyan-50 !tw:text-cyan-700': item.discountType === 'E'
                            }">
                            <Icon :icon="item.discountType === 'P' ? 'mdi:percent' : 'mdi:cash-multiple'"
                                class="mr-1" height="16" />
                    ...
                    <template #item.needSupervision="{ item }">
                        <VChip :color="item.needSupervision ? 'warning' : 'success'" size="small" variant="tonal"
                            class="!tw:font-medium tw:text-xs !tw:min-w-[100px] tw:justify-center" :class="{
                                'tw:bg-amber-50 !tw:text-amber-700': item.needSupervision,
                                'tw:bg-green-50 !tw:text-green-700': !item.needSupervision
                            }">
                            <Icon :icon="item.needSupervision ? 'mdi:shield-account' : 'mdi:shield-check'"
                                class="mr-1" height="16" />
                    ...
                    <template #item.isActive="{ item }">
                        <VChip :color="item.isActive ? 'success' : 'error'" size="small" variant="flat"
                            ...
                            <Icon :icon="item.isActive ? 'mdi:check-circle' : 'mdi:close-circle'" class="mr-1"
                                height="16" />
                    ...
                    <template v-slot:item.actions="{ item }">
                        <div class="d-flex ga-2">
                            <v-btn icon color="info" variant="text" size="32"
                                ...
                                v-tooltip="'Editar Descuento'" @click="onEditDiscount(item.id)">
    ...
    <CreateDiscount v-model:modelValue="showCreateDialog" :discount="selectedDiscount" @save="handleSaveDiscount"
        @cancel="showCreateDialog = false" />
    <EditDiscount v-if="selectedDiscount" v-model:modelValue="showEditDialog" :discount="selectedDiscount"
        @save="handleUpdateDiscount" @cancel="showEditDialog = false" />
</template>
