...
<template>
    <BaseBreadcrumb :title="'Crear Nuevo Cobro'" :breadcrumbs="breadcrumbs">
        <template #action>
            <v-btn color="primary" @click="processPayment" :loading="isLoading"
        ...
        <v-row>
            <v-col cols="12" md="6">
                <v-card class="mb-4 payment-card">
                    ...
                    <v-card-text class="text-center"> Serás redirigido en {{ redirectCountdown }} segundos...
                    </v-card-text>
                    ...
                    <v-row>
                            ...
                            <v-card class="mb-4 payment-card">
                                ...
                                <v-card-text>
                                    <GeneralPayment @update:selected-participant="selectedParticipant = $event"
                                        ...
                                        @update:notes="notes = $event" @update:fullname="fullname = $event"
                                        @update:address="address = $event" @update:document="document = $event"
                                        @update:phone="phone = $event" @update:email="email = $event"
                        ...
                        <v-col cols="12" md="6">
                            <v-card class="mb-4 h-full">
                                ...
                                <v-card-text>
                                    <PaymentPreview :payment-number="paymentNumber" :billed-by="billedBy"
                                        :billed-to="billedTo" :date-issued="dateIssued" :due-date="dueDate"
                                        :items="invoiceItems" :discount-amount="discountAmount" :notes="notes" />
                                </v-card-text>
                            </v-card>
                        </v-col>
                    </v-row>
