...
const onToggleUserStatus = (user: Admin) => {
  ...
  Swal.fire({
    ...
    text: `Estás a punto de ${action} el Usuario ${user.user.name}. ¿Deseas continuar?`,
    icon: 'warning',
...
<template>
  ...
  <div v-if="checkPermission(PermissionEnum.SEE_USERS)">
    <v-row>
      <v-col cols="12" md="12">
            ...
            >
              <template v-slot:top>
                ...
                >
                  ...
                  <v-spacer></v-spacer>
                  <VBtn
                    :to="{ name: 'users-admin-add' }"
                    color="primary"
                    variant="elevated"
                    v-if="checkPermission(PermissionEnum.CREATE_USERS)"
                  >
              ...
              <template v-slot:item.email="{ item }">
                <div class="tw:flex tw:items-center tw:gap-3 tw:text-nowrap group">
                  ...
                  <div>
                    <v-chip>
                      <Icon icon="mdi:email" class="mr-2 tw:text-primary tw:transition-transform group-hover:tw:scale-110" />
                      <span class="tw:font-medium tw:text-gray-800 group-hover:tw:text-primary tw:transition-colors">
                        {{ item.email }}
                      </span>
                    </v-chip>
              ...
              <template v-slot:item.actions="{ item }">
                <div class="d-flex ga-2">
                  <v-btn
                    v-if="checkPermission(PermissionEnum.UPDATE_USERS)"
                    icon
                    color="info"
                    variant="text"
                    size="32"
    ...
    <VDialog v-model="showForm" max-width="500" v-if="selectedUser">
      <VCard>
        <VCardTitle>
          <h3>Modificar Administrador: {{ selectedUser.user.name }}</h3>
        </VCardTitle>
